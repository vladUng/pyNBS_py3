...
def network_propagation(network, binary_matrix, alpha=0.7, symmetric_norm=False, verbose=True, **save_args):
    ...
    starttime = time.time()
    if verbose:
        print(f"Performing network propagation with alpha: {alpha}")
    ...
    if "outdir" in save_args:
        ...
        if verbose:
            print(f"Network Propagation Result Saved: {save_path}")
    else:
        pass
    if verbose:
        print(f"Network Propagation Complete: {time.time()-starttime} seconds")
    return prop_data_df
...
def network_kernel_propagation(network, network_kernel, binary_matrix, verbose=False, **save_args):
    ...
    prop_data = np.dot(binary_matrix.T.loc[prop_nodelist].fillna(0).T, network_kernel.loc[prop_nodelist][prop_nodelist])
    # Get propagated results for remaining subgraphs
